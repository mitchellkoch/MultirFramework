...
public class DefaultFeatureGeneratorMinusDirMinusDepMinusLongGeneralized implements FeatureGenerator{
	...
	private static final String DEP_Feature = "dep:";
        ...
        private static DefaultFeatureGeneratorMinusDirMinusDepMinusLong OldFeatureGenerator = new DefaultFeatureGeneratorMinusDirMinusDepMinusLong();
...
//		rightGeneralizedTokenSequenceFeature = makeSequenceFeature(generalRightTokens2.subList(0,Math.min(generalRightTokens2.size(), 3)),RIGHT_PREFIX);
		...
		if(dependencyPathMiddleTokens.size() > 0){
			...
			for(Triple<CoreLabel,DependencyType,CoreLabel> depWindow : dependencyPathLeftWindow){
				...
				if(isGeneralized(feature)) features.add(feature);
				feature = generalizedMiddleDependencySequenceFeature + makeFeature(dependencyPathSingleFeature(depWindow,GeneralizationClass.Second),LEFT_PREFIX);
				if(isGeneralized(feature)) features.add(feature);
				feature = generalizedMiddleDependencySequenceFeature2+ makeFeature(dependencyPathSingleFeature(depWindow,GeneralizationClass.First),LEFT_PREFIX);				
				if(isGeneralized(feature)) features.add(feature);
			}
			for(Triple<CoreLabel,DependencyType,CoreLabel> depWindow : dependencyPathRightWindow){
				...
				if(isGeneralized(feature)) features.add(feature);
				feature = generalizedMiddleDependencySequenceFeature2 + makeFeature(dependencyPathSingleFeature(depWindow,GeneralizationClass.First),RIGHT_PREFIX);
				if(isGeneralized(feature)) features.add(feature);
				feature = generalizedMiddleDependencySequenceFeature + makeFeature(dependencyPathSingleFeature(depWindow,GeneralizationClass.Second),RIGHT_PREFIX);
				if(isGeneralized(feature)) features.add(feature);
			}
		}
		...
		for(int i = 0; i < dependencyList.size(); i++){
			...
			if(i!=dependencyList.size()-1){
				...
				case First:
					...
					break;
				case Second:
					  List<CoreLabel> token2List = new ArrayList<>();
					  token2List.add(trip.third);
					  String ner2String = FeatureGeneratorMethods.findHeadEntityType(token2List);
					  if(ner2String!=null){
						  text = "N(" + ner2String + ")";
					  }
					  else{
							String lemma2 = FeatureGeneratorMethods.getWordnet2NounLemma(trip.third.get(CoreAnnotations.TextAnnotation.class));
							if(lemma2 != null){
								text = "N("+lemma2+")";
							}
					  }
    ...
    private String dependencyPathSingleFeature(Triple<CoreLabel, DependencyType, CoreLabel> depInfo,
		...
		case Second:
			  List<CoreLabel> token2List = new ArrayList<>();
			  token2List.add(depInfo.third);
			  String ner2String = FeatureGeneratorMethods.findHeadEntityType(token2List);
			  if(ner2String!=null){
				  text = "N(" + ner2String + ")";
			  }
			  else{
				String lemma2 = FeatureGeneratorMethods.getWordnet2NounLemma(depInfo.third.get(CoreAnnotations.TextAnnotation.class));
				if(lemma2 != null){
					text = "N("+lemma2+")";
				}
			  }
